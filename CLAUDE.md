...
## 📌 報告のしかた：何か変えたら、毎回URLを貼る

**恒久ルール。本人の指示（2026-08-22）。「遡って見つけるのが手間」だから、
返事の中に毎回そのまま置く。「さっき貼った」「PRを見てください」は無し。**

何かを直した・足した・消したときは、その回の返事の最後に必ずこの3つを並べる。

```
📄 PR      https://github.com/takayasuuchida/spectralayers/pull/<番号>
🔁 ボード   https://takayasuuchida.github.io/spectralayers/board.html?v=<日付など>
📖 ガイド   https://takayasuuchida.github.io/spectralayers/board-manual.html?v=<日付など>
```

- **触ったページのURLは全部貼る。** board.html を直したらボードのURL、
  ガイドも直したならガイドのURLも。触っていないページは省いてよい。
- **公開URLには `?v=` を付ける。** 付けないと端末のキャッシュで
  古い画面が出て「変わってねーぞ」になる（実際になった）。値は毎回変える。
- **マージ前なら「マージするとここに出る」と添えて貼る。** リンク自体は必ず出す。
- つけ回しツール本体（`index.html`）を触ったときは
  `https://takayasuuchida.github.io/spectralayers/` も貼る。
